...
<body>
	...
	<!-- 载入公共头部文件-->
	<link href="/app/shop/Public/Member/css/cart.css" type="text/css" rel="stylesheet" >
	<div id="main">
		<?php if(isset($cart)): ?><div class='step'>
		...
		<!-- 购物车列表 -->
		<form action="<?php echo U('Buy/payment');?>" method="post">
			...
			<tbody>
				...
				<tr>
					...
					<td class='goods-num'>
						...
						<input class="goodsNum" name="goods_num[]" gid="<?php echo ($v["gid"]); ?>" url="<?php echo U('Cart/updateGoodsNum');?>" type="text" value=<?php echo ($v["goods_num"]); ?>> 
						<a href="javascript:void(0);" class='add addGoods' ></a>
					</td>
					<td id="price"><?php echo ($v["price"]); ?></td>
					<td id="subtotal"><?php echo ($v["subtotal"]); ?></td>
					<td><a class="btn btn-primary btn-xs active" role="button" style="background-color: #29B4A8;" href="<?php echo U('Cart/delgoods');?>/gid/<?php echo ($v["gid"]); ?>">删除</a></td>
			...
			<table>
				...
				<tbody>
					<?php if(is_array($address)): foreach($address as $key=>$v): ?><tr>
						...
						<td>
							<a href="<?php echo U('Account/delAddress');?>/addressid/<?php echo ($v["addressid"]); ?>">删除</a>
						</td>
					</tr><?php endforeach; endif; ?>
				</tbody>
			</table>	
		...
		<?php else: ?>
			没有添加到购物车的商品。<a href="/app/shop">去看看</a><?php endif; ?>
	</div>
	<script src="/app/shop/Public/Member/js/cart.js"></script>
</body>
