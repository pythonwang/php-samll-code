...
<div id="main">
	<div class='left'>
		...
		<div id="content">
		<link href="/app/shop/Public/Member/css/order.css" type="text/css" rel="stylesheet" >
		<div class='order-nav'>
		<?php if(is_array($menu)): foreach($menu as $key=>$v): echo ($v); endforeach; endif; ?>
		</div>
				...
				<tbody>
					...
					<?php if(is_array($order)): foreach($order as $key=>$v): ?><tr>
						...
						<td>
							...
							<a href="<?php echo U('Home/Detail/index');?>/gid/<?php echo ($v["gid"]); ?>" >订单详情</a>
						</td>
						<td>
							<?php if($v['status'] == '未付款'): ?><a class='btn' href="<?php echo U('Member/Buy/payment');?>/oid/<?php echo ($v["orderid"]); ?>">付款</a><br/><?php endif; ?>
