...
$(function () {
	...
	$('#picture').uploadify({
			...
			onUploadSuccess : function (file,data,response) {
				...
				$('input[name=max]').val(info.path.max);
				$('input[name=medium]').val(info.path.med);
				$('input[name=mini]').val(info.path.mini);
				$('#upload_img').fadeOut().next().fadeIn().find('img').attr('src',ROOT + '/Uploads/' + info.path.med);
	 ...
	 */
	 $('.turn').click(function () {
	 	...
	 	$('form[name=turn] input[name=tid]').val($(this).attr('tid'));
